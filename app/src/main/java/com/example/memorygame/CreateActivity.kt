...
class CreateActivity : AppCompatActivity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode != PICK_PHOTO_CODE || resultCode != Activity.RESULT_OK || data == null) {
            Log.w(
                TAG,
                "Did not get the data back from the launched activity, user likely canceled flow"
            )
            return
        }
    ...
    private fun saveDataToFirebase() {
        ...
        // CHECK THAT WE'RE NOT OVER WRITING SOMEONE'S ELSE'S DATA
        db.collection("games").document(customGameName).get().addOnSuccessListener { document ->
            if (document != null && document.data != null) {
                AlertDialog.Builder(this)
                    .setTitle("Name taken")
                    .setMessage("A game alread exists with same name '$customGameName'.please choose another")
            ...
            Toast.makeText(this, "Encounter error while saving memory game ", Toast.LENGTH_LONG)
                .show()
        ...
        for ((index, photoUri) in chosenImageUris.withIndex()) {
            ...
            photoReference.putBytes(imageByteArray)
                .continueWithTask { photoUploadTask ->
                    Log.i(TAG, "Uploads bytes : ${photoUploadTask.result?.bytesTransferred}")
                    photoReference.downloadUrl

                }.addOnCompleteListener { downloadUriTask ->
                    if (!downloadUriTask.isSuccessful) {
                        Log.e(TAG, "Exception with firebase storage", downloadUriTask.exception)
                    ...
                    val downloadUri = downloadUriTask.result.toString()
                    uploadImageUrls.add(downloadUri)
    ...
    private fun handleAllImagesUploaded(gameName: String, imageUris: MutableList<String>) {
        ...
        db.collection("games").document(gameName)
            .set(mapOf("images" to imageUris))
            .addOnCompleteListener { gameCreationTask ->
                pbUploading.visibility=View.GONE
                if (!gameCreationTask.isSuccessful) {
                    Log.e(TAG, "Excetion with the game creation ", gameCreationTask.exception)
